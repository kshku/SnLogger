...
static size_t format_layout(LayoutCtx *ctx, uint64_t timestamp, snLogLevel level, const char *msg, size_t msg_len) {
    ...
    // NOTE: If VT is not enabled, colored output is not visible in windows and other characters
    // will be displayed instead!
    static const char *colors[] = {"0;37", "0;34", "0;32", "0;33", "1;31", "1;41"};

    int n = snprintf(ctx->buffer, sizeof(ctx->buffer), "\x1b[%sm[%llu] [%s]: ", colors[level],
                     (unsigned long long)timestamp, level_str);
#else
    ...
    size_t copy = msg_len;
    if (off + copy > sizeof(ctx->buffer)) copy = sizeof(ctx->buffer) - off;
...
int main(void) {
    ...
    sn_static_logger_log(&logger, SN_LOG_LEVEL_FATAL, "Hello %s", "FATAL");
